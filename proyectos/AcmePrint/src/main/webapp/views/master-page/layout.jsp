...
<link rel="stylesheet" href="styles/folder.css" type="text/css">
...
<script type="text/javascript">
	...
	function phoneValidation(phoneConfirmationPrefix, phoneConfirmationSuffix){
		var PhoneInput = document.getElementById('phoneId').value;
		
			var patt = new RegExp(
					"^(\\+[1-9][0-9]{2}|\\+[1-9][0-9]|\\+[1-9])(\\s\\([1-9][0-9]{2}\\)|\\"
							+ " \\([1-9][0-9]\\)|\\ \\([1-9]\\))?(\\ \\d{4,})|(\\d{4,})$");
			if (PhoneInput != "" && !patt.test(PhoneInput)) {
				var m = phoneConfirmationPrefix + " " + PhoneInput + " " + phoneConfirmationSuffix;
				if (confirm(m)){
					return true;
				} else {
					return false;
