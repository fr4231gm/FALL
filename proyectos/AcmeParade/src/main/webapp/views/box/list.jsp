...
<%-- Lista De Carpetas --%>
		...
		<tr>
			<td>
				...
				<a href="box/list.do?boxId=${box.id}">${box.name} (${fn:length(box.messages)})</a>
					...
					<input type="button" name="edit" id="edit" value="<spring:message code="message.edit" />" onclick="javascript: relativeRedir('box/edit.do?id=${box.id}');" />
					</jstl:if>
			</td>
		</tr>
		</jstl:forEach>
	...
	<display:table name="${childboxes}" id="row"
		class="displaytag" >
				...
				<input type="button" name="edit" id="edit" value="<spring:message code="message.edit" />" onclick="javascript: relativeRedir('box/edit.do?id=${row.id}');" />
