...
public class BoxService {
	...
	public Collection<Box> generateDefaultFolders(Actor x) {
		...
		final String[] systemboxesnames = new String[5];
		systemboxesnames[0] = "InBox";
		systemboxesnames[1] = "OutBox";
		systemboxesnames[2] = "TrashBox";
		systemboxesnames[3] = "SpamBox";
		systemboxesnames[4] = "NotificationBox";
	...
	public void deleteByUserDropOut(Actor principal) {
		List<Box> boxesToDelete = new ArrayList<Box>(this.boxRepository.findRootBoxesByActor(principal.getId()));
		for(int i = 0; i<boxesToDelete.size(); i++){
			Box b = boxesToDelete.get(i);
			
			List<Box> childboxes = new ArrayList<Box>(this.getchilds(b.getId()));
			List<Message> messages = new ArrayList<Message>(b.getMessages());
			
			for (int k = 0; k < childboxes.size(); k++) {
				this.delete(childboxes.get(k));
			}
			for (int j = 0; j < messages.size(); j++) {
				this.messageService.deleteByUserDropOut(messages.get(j));
			}
			this.boxRepository.delete(b);
		}
	}

	public Box save2(Box b) {
		return this.boxRepository.save(b);
