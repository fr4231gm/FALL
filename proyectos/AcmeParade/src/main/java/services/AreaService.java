...
public class AreaService {
	...
	public Collection<Area> findWithoutChapter() {
		Collection<Area> res;

		res = this.areaRepository.findWithoutChapter();
		Assert.notNull(res);

		return res;
	}

	public Area findByChapter() {
		final Chapter c = this.chapterService.findByPrincipal();
		final Area res = this.areaRepository.findByChapter(c.getId());
		return res;
	...
	public Area findAreaByChapterId(int chapterId) {
		
		Assert.notNull(chapterId);
		
		Area res = this.areaRepository.findByChapter(chapterId);
