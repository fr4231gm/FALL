...
public class FinderController extends AbstractController {
	...
	public ModelAndView allPositions() {
		...
		if (finder.getId() != 0)
			finder.setVersion(this.hackerService.findByPrincipal().getFinder().getVersion());
		...
		final String requestURI = "finder/hacker/list.do";
	...
	public ModelAndView edit() {
		...
		finder = this.hackerService.findByPrincipal().getFinder();
	...
	public ModelAndView save(@Valid final Finder finder, final BindingResult binding) {
		...
		if (finder.getId() != 0)
			finder.setVersion(this.hackerService.findByPrincipal().getFinder().getVersion());
		final Finder finderf = this.hackerService.findByPrincipal().getFinder();
		final Date lastUpdate = finder.getLastUpdate();
		final Date endTime = new Date();
		final int MILLI_TO_HOUR = 1000 * 60 * 60;
		Integer period = 0;
		...
		if (binding.hasErrors()) {
			...
			try {
				if (finder.getVersion() == 0)
					this.finderService.saveFinder(finder);
				else if (finder.getKeyword().equals(finderf.getKeyword()) && finder.getDeadline().equals(finderf.getDeadline()) && finder.getMaximumSalary().equals(finderf.getMaximumSalary()) && finder.getMinimumSalary().equals(finderf.getMinimumSalary())) {
					if (period > this.configurationService.findConfiguration().getFinderLifeSpan())
						this.finderService.saveFinder(finder);
				} else
					this.finderService.saveFinder(finder);
