...
public class CompanyService {
	...
	public Company save(final Company company) {
		...
		if (company.getId() == 0) {
			...
			company.getUserAccount().setPassword(
					passwordEncoder.encodePassword(company.getUserAccount()
							.getPassword(), null));
			...
			Assert.isTrue(principal.getUserAccount() == company
					.getUserAccount());
		}
	...
	// SAVE
	public Company saveFirst(final CompanyForm form,BindingResult binding) {
		Company company = this.reconstruct(form, binding);
	...
	public Company reconstruct(final CompanyForm companyForm,
		...
		if (encaja.find())
			companyForm.setPhoneNumber(this.configurationService
					.findConfiguration().getCountryCode()
					+ companyForm.getPhoneNumber());

		// Declarar variables
		Company company;

		// Crear un nuevo Company
		company = this.create(companyForm.getUsername(),
				companyForm.getPassword());
		...
		if (!companyForm.getPassword().equals(
				companyForm.getPasswordConfirmation()))
	...
	public Company reconstruct(final Company company,
			final BindingResult binding) {
		Company result;
		Company aux;
