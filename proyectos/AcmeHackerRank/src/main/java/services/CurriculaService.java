...
public class CurriculaService {
	...
	public Curricula create() {
		// Comprobamos que la persona logueada es unhacker y no tiene ya
		// una curricula
		Hacker principal;
		...
		res.setHacker(principal);
		return res;
	}

	public Curricula save(final Curricula curricula) {
		// Comprobamos que la persona logueada es una hacker
		Hacker principal;
		Curricula res;
		principal = this.hackerService.findByPrincipal();
		Assert.notNull(principal);
		...
		if (curricula.getId() == 0)
			...
			// Comprobamos que le pertenece la curricula que se est� editando
	...
	public Curricula findCopiedById(int id) {
		Curricula res = this.curriculaRepository.findCopiedById(id);
		return res;
	}
	...
	public Collection<Curricula> findByPrincipal() {
		final Hacker b = this.hackerService.findByPrincipal();
		Collection<Curricula> res;
		res = this.curriculaRepository.findByHackerId(b.getId());
		return res;
	}
	...
	public Collection<Curricula> findByHackerId(final int id) {
		return this.curriculaRepository.findByHackerId(id);
	}
	...
	public Curricula findOneToEdit(final int curriculaId) {
		...
		Curricula res;
		res = this.curriculaRepository.findOne(curriculaId);
		Assert.isTrue(res.getHacker() == principal);
		return res;
	}
