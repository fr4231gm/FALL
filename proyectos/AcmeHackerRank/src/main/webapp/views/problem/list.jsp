...
<display:table name="problems" id="row" requestURI="${requestURI}"
	...
	<display:column titleKey ="problem.display">
		<acme:link link="problem/display.do?problemId=${row.id}"
	...
	<jstl:if test="${row.isDraft == true}">
		<acme:link link="problem/edit.do?problemId=${row.id}" 	code ="problem.edit"/>
	</jstl:if>
	
	</display:column>
	
	<display:column titleKey="problem.delete">
	<acme:link link="problem/delete.do?problemId=${row.id}" code ="problem.delete"/>
	</display:column>
...
<p style="color:red;"><spring:message code="${messageError}"></spring:message></p>
	...
	<security:authorize access="hasRole('COMPANY')">
	<acme:link code ="problem.create" link="problem/create.do"/>
	</security:authorize>
