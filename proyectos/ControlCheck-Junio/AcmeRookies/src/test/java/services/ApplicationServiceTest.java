...
public class ApplicationServiceTest extends AbstractTest {
	...
	// 1. Listar applications siendo rookie

	// In the case of negative tests, the business rule that is intended to be
	// broken:
	// 2. Listar applications de un rookie siendo company

	// Analysis of sentence coverage total: Covered 2.2% 10/456 total instructions
	// Analysis of sentence coverage listApplicationsByRookie(): Covered 100% 7/7 total instructions

	// Analysis of data coverage: 100%
	@Test
	public void listApplicationByRookieTestDriver() {

		final Object testingData[][] = {

			// 1. TEST POSITIVO: Listar applications siendo rookie
			{
				"rookie1", null
			},

			// 2. TEST NEGATIVO: Listar applications de un rookie siendo company
			{
		...
		for (int i = 0; i < testingData.length; i++) {
			...
			this.listApplicationRookieTemplate((String) testingData[i][0], (Class<?>) testingData[i][1]);
			this.rollbackTransaction();
		}
	}

	protected void listApplicationRookieTemplate(final String username, final Class<?> expected) {
		Class<?> caught;
		...
		try {
			...
			this.applicationService.findApplicationsByRookie(rookie.getId());
	...
	// In the case of negative tests, the business rule that is intended to be
	// broken:
	// 2. Listar applications de una company siendo rookie
	...
	public void listApplicationByCompanyTestDriver() {
		...
		final Object testingData[][] = {
			...
			// TEST NEGATIVO: Un rookie intenta listar applications de una company
			{
	...
	// In the case of negative tests, the business rule that is intended to be
	// broken:
	// 2. Un rookie intenta aceptar una application con estado submitted
	...
	public void acceptApplicationTestDriver() {
		...
		final Object testingData[][] = {
			...
			// 2. TEST NEGATIVO: 
			{
				"rookie1", super.getEntityId("application3"), IllegalArgumentException.class
			},
	...
	// In the case of negative tests, the business rule that is intended to be
	// broken:
	// 2. Un rookie intenta rechazar una application con estado submitted
	...
	/** Submit application **/
	// Requierement: Manage the applications to their positions, which includes listing them grouped by status, showing them, and updating them. Updating an application amounts to making a decision on it: an application whose status is SUBMITTED may change to status ACCEPTED or REJECTED.
	// 1. Un rookie env�a una application con estado pending

	// In the case of negative tests, the business rule that is intended to be
	// broken:
	// 2. Una company intenta enviar una application con estado pending
	// 3. Un rookie env�a una application con estado distinto de submitted
	...
	public void submitApplicationTestDriver() {
		...
		final Object testingData[][] = {
			...
			// 1. TEST POSITIVO:
			{
				"rookie8", super.getEntityId("application6"), null
			},
	...
	// 1. Un rookie intenta crear una solicitud
	...
	public void createApplicationTestDriver() {
		...
		final Object testingData[][] = {
			...
			// 1. TEST POSITIVO: 
			{
				"rookie1", super.getEntityId("position1"), null
			},
	...
	// Requierement: Manage his or her applications, which includes listing them grouped by status, 
	// showing them, creating them, and updating them. When an application is created, the system assigns an arbitrary problem to it (from the set of problems that have been registered for the corresponding position). Updating an application consists in sub- mitting a solution to the corresponding problem (a piece of text with explanations and a link to the code), registering the submission moment, and changing the status to SUBMITTED.
	// 1. Un rookie edita una solicitud suya con estado pending

	// In the case of negative tests, the business rule that is intended to be
	// broken:
	// 2. Un rookie intenta editar una solicitud suya con estado no pending
	// 3. Un rookie intenta editar una solicitud que no es suya con estado pending
	...
	public void editApplicationTestDriver() {
		...
		final Object testingData[][] = {
			...
			{
				"rookie7", super.getEntityId("application5"), IllegalArgumentException.class
			},

			// 3. TEST NEGATIVO: 
			{
				"rookie7", super.getEntityId("application6"), IllegalArgumentException.class
			}
