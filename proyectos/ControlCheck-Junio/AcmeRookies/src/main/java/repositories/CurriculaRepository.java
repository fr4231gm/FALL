...
public interface CurriculaRepository extends JpaRepository<Curricula, Integer> {
	...
	@Query("select h from Curricula h where h.rookie.id = ?1")
	...
	@Query("select h from Curricula h where h.rookie.id = ?1 and h not in (select p.curricula from Application p)")
