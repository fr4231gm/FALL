...
public class ApplicationService {
	...
	public Application submit(final Application application) {
		...
		principal = this.rookieService.findByPrincipal();
		Assert.notNull(principal);
		Assert.isTrue(application.getRookie() == principal);
	...
	public Application accept(final Application application) {
		...
		this.messageService.notificateApplicationStatusChange(application.getPosition().getCompany(), application.getRookie(), "Submitted", "Accepted", application.getPosition().getTitle());
	...
	public Application reject(final Application application) {
		...
		this.messageService.notificateApplicationStatusChange(application.getPosition().getCompany(), application.getRookie(), "Submitted", "Rejected", application.getPosition().getTitle());
	...
	public Collection<Application> findApplicationsByRookie(final int rookieId) {
		Collection<Application> result;

		result = this.applicationRepository.findApplicationsByRookie(rookieId);
	...
	public Application reconstruct(final ApplicationForm applicationForm, final BindingResult binding) {
		...
		if (applicationForm.getId() != 0) {
			application.setRookie(this.applicationRepository.findOne(applicationForm.getId()).getRookie());
	...
	public void deleteByUserDropOut(Application application) {
