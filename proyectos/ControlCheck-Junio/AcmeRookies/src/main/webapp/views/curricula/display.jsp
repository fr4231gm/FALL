...
<jstl:if test="${curricula.rookie.id == principal.id }">
	<table class="displayStyle">
		...
		<tr>
			<td><input type="button" name="addMR"
				...
				onclick="redirect: location.href = 'miscellaneousData/rookie/create.do?curriculaId=${curricula.id}';" />
			</td>

			<td><input type="button" name="addER"
				value="<spring:message code="curricula.addER" />"
				onclick="redirect: location.href = 'educationData/rookie/create.do?curriculaId=${curricula.id}';" />
			</td>
			<td><input type="button" name="addPD"
				value="<spring:message code="curricula.addPD" />"
				onclick="redirect: location.href = 'positionData/rookie/create.do?curriculaId=${curricula.id}';" />
			</td>
		</tr>
				...
				<jstl:if
					...
					<tr>
						<td colspan="2"><br> <input type="button" name="edit"
							...
							onclick="redirect: location.href = 'curricula/rookie/edit.do?curriculaId=${curricula.id}';" />
						</td>
					</tr>
				</jstl:if>
			</table>
					...
					<jstl:if
						...
						<display:column titleKey="curricula.editER" sortable="true">
							<input type="button" name="edit"
								...
								onclick="redirect: location.href = 'educationData/rookie/edit.do?educationDataId=${row2.id}';" />
						</display:column>
					</jstl:if>
				</display:table></td>
		</tr>
					...
					<jstl:if
						...
						<display:column titleKey="curricula.editMR" sortable="true">
							<input type="button" name="edit"
								...
								onclick="redirect: location.href = 'miscellaneousData/rookie/edit.do?miscellaneousDataId=${row.id}';" />
						</display:column>
					</jstl:if>
				</display:table></td>
		</tr>
...
<div>
	<jstl:if test="${curricula.rookie == principal}">
