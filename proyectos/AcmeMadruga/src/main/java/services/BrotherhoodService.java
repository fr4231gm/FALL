...
public class BrotherhoodService {
	...
	// CRUDS Methods ----------------------
	public Brotherhood create(String username, String password) {
		// Nueva brotherhood
	...
	public Brotherhood save(final Brotherhood brotherhood) {
		...
		// Si la hermandad no est� en la base de datos (es una creacion, no
		// una actualizacion)
		...
		if (brotherhood.getId() == 0) {
			...
			Assert.isTrue(principal.getUserAccount() == brotherhood
					.getUserAccount());
		}
	...
	public Brotherhood findBrotherhoodByUserAccount(final int userAccountId) {
		...
		result = this.brotherhoodRepository
				.findBrotherhoodByUserAccount(userAccountId);
	...
	public Collection<Brotherhood> findBrotherhoodsWhithoutEnrolmentByPrincipal() {
		...
		res = this.brotherhoodRepository
				.findBrotherhoodsNotEnrolledByMember(principal.getId());
	...
	public Collection<Brotherhood> findBrotherhoodsEnrolledByPrincipal() {
		...
		// Obtengo todas las brotherhoods
		res = this.brotherhoodRepository
				.findBrotherhoodsEnrolledByMember(principal.getId());
	...
	public Brotherhood reconstruct(BrotherhoodForm form, BindingResult binding) {
	...
	public Brotherhood construct(Brotherhood brotherhood) {
	...
	public Brotherhood findOneTrimmedByPrincipal() {
		// Initialize variables
		Brotherhood result;
		Brotherhood principal = this.findByPrincipal();
	...
	public Brotherhood reconstruct(Brotherhood brotherhood,
		...
		if (!(brotherhood.getEmail()
				.matches("[A-Za-z_.]+[\\w]+[\\S]+@[a-zA-Z0-9.-]+|[\\w\\s]+[\\<][A-Za-z_.]+[\\w]+@[a-zA-Z0-9.-]+[\\>]"))
				&& brotherhood.getEmail().length() > 0)
			binding.rejectValue("email", "member.email.check");
		if (brotherhood.getArea() == null)
			binding.rejectValue("area", "brotherhood.area.check");
		// Checking that all the pictures URL are valid
		if (checkPictures(brotherhood.getPictures())) {
			binding.rejectValue("pictures", "brotherhood.pictures.invalid");
		}
		return result;
	}
