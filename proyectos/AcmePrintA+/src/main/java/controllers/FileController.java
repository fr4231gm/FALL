...
public class FileController extends AbstractController {
	...
	public ModelAndView downloadDocument(@RequestParam int fileId, HttpServletResponse response) throws IOException {
		...
		Assert.isTrue(file.getInvoice().getApplication().getOrder().getCustomer().getId() == principal.getId());
		res = this.getRedirect(file);
		}
		catch(Throwable oops){
	...
	private ModelAndView getRedirect(File file) {
		...
		if (file.getInvoice() != null) {
			return new ModelAndView("redirect:/invoice/display.do?invoiceId=" + file.getInvoice().getId());
		}
